...
| Curriculum | `database_service.dart`, home curriculum views. | Reviewed units/lessons/flashcards plus versioned bundled fallback. | P1: loading/empty/error distinct; current language only; cache version test. |
| Flashcard/SRS | **`flashcard_study_session.dart` only**. | `card_reviews`, `complete_lesson`, lesson progress, server ownership/RLS. | P1: correct/incorrect progression, retry, write then refresh, cross-account isolation. |
